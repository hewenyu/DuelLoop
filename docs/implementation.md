...
实施基线：[设计 v0.3](design.md)。截至 2026-09-22，首个 `0.1.0` 开发实现已具备公开 SDK、CLI、模型适配、双循环、SQLite、领域扩展和独立消费演示。已完成 v0.3 定义的首版本地交付与受控模拟验收，可从源码构建、本地打包、安装并运行；这是单机开发版本，尚未发布到 npm，也不表示任意真实业务环境已达到生产要求。

项目已采用 [MIT 许可证](../LICENSE)，保留 `private: true`，尚未发布到 npm。真实 `jev-1.13.0` 的 Score/Choice 调用及通过 pi 使用 `deepseek-flash` 的受控工具调用已通过，可公开结果见[验证摘要](validation.md)；原始模型输出与本机验收数据仅在本地保存。这确认了线上传输和工具调用可用；真实顺序和并发性能另行测量，研究收益仍需独立实验判断。接口使用锁定的官方 SDK；夹具、本机 HTTP/SSE 和真实服务证据分别记录。
...
| 快循环与执行 | `src/runtime.ts` | 托管或宿主执行、统一模式检查、截止时间、领域降级、意图/回执、未知执行核对 |
...
| I-04 统一执行检查 | 策略、领域基线、强制动作共用执行路径；宿主/影子不隐式执行；数据库原子领取意图，重复并发提交不能发送两次 | `test/core.test.mjs`、`test/process-recovery.test.mjs` |
...
| R9 维护与升级 | Schema 1 只读备份校验、恢复及存储限额通过 | core/CLI/storage-maintenance/storage-limits 测试、[运维说明](operations.md) | 尚无历史生产 Schema 迁移；只拒绝未知更新版本，不支持任意降级 |
...
允许 `no_change`、验证拒绝或证据不足。真实受控更新实验未通过时保留当前策略和原门槛，不为了完成里程碑强制生成新版本。
