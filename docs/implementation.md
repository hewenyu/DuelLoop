...
当前行为基线：[模型决策，失败停止](model-only-decisions.md)及[修订设计](design.md)。策略 Schema 和评价协议为 `2.0`，运行时为 `duelloop-runtime-3`。每个执行动作依赖当次模型回答；失败持久记录并停止，单候选不跳过模型。

2026-09-23 本轮实现通过 147 项离线测试及独立安装检查：57 个公开类型、12 步 SDK 演示、安装包文档和领域模板均验证。领域合规 11 项通过、2 个未提供可选场景跳过；本轮未调用收费模型，完整范围见[当前验证摘要](validation.md)。
