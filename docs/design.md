...
状态：总体架构与开发者产品范围保留；决策契约按[模型决策，失败停止](model-only-decisions.md)修订。已有 0.1.0 实现；旧语义的首版本地交付与真实模型受控闭环验收已完成；不作为本轮真实模型验证，采用 MIT，尚未发布到 npm。这是单机开发版本；[能力范围](validation.md)区分可用接口、实验用途和生产接入职责。五项实现审查点和 R1—R10 实际证据见[实施跟踪](implementation.md)。
...
| 策略可演化 | 慢循环能修改条件、评估维度、权重、条件阈值和问题模板 |
...
| `DecisionRecord` | 决策 ID、观察快照、候选集、策略及模型版本、问题与答案、组合结果、决策来源、停止原因、最终动作 | 解释当时如何从模型策略走到动作或停止 |
...
`ActionCommand` 必须携带 `decisionId`、`idempotencyKey`、`expectedStateRevision`、动作及截止时间。适配器应在提交时检查状态修订号与动作合法性。单候选也调用模型，无候选则停止并报告原因；领域不提供模型失败接管接口。
...
领域以 `DomainDefinition` 注册，包含领域/规则版本、观察和特征 Schema、动作契约、决策上下文、适配器工厂、切换边界及可用评价能力。初始策略、示例配置和测试夹具随模板提供。领域包可直接位于用户应用中，不要求独立服务或公开发布到包仓库。
...
可安装的合规测试套件供外部项目运行，覆盖合法性、过期状态、无动作/单候选、重复提交、未知回执、延迟与修订反馈、激活边界、知识隔离和场景重置。可选能力未声明时报告跳过及因此禁用的功能，不把跳过记为能力验证成功。测试失败应定位契约、输入夹具及原因。
...
第一版采用受约束的声明式策略语言，由确定性解释器执行。JSON 是规范存储格式，YAML 可作为编辑格式并转换为 JSON。策略不包含任意 JavaScript、Python、网络请求或文件操作。当前策略 Schema 为 `2.0`，评价协议为 `2.0`，运行时行为版本为 `duelloop-runtime-3`；旧字段与旧行为绑定明确拒绝，历史证据保留。
...
| 适用范围 | `scope`、规则与特征契约版本 | 不匹配时停止并报告错误 |
...
第一版条件运算符限定为 `eq`、`gt`、`gte`、`lt`、`lte`、`in` 和 `all/any/not`。特征缺失或未知产生三值逻辑中的 `unknown`，`not unknown` 仍为 `unknown`；只有条件结果为 `true` 才进入分支。输入投影中由领域特征契约声明为必填的特征缺失时停止决策，策略不能放宽必填约束。

本例契约将 `opponent.callOpportunities` 声明为必填非负整数，将 `opponent.observedCallRate` 声明为可选数值。冷启动时两者分别为 `0` 和 `null`，分支不命中，使用默认权重；不会仅因缺少跟注率而停止。公共问题指令要求 Jev 将 `null` 解释为未知，基于当前牌局和规则评分，不虚构对手特征；合法低置信度评分仍正常参与组合，不设置接受门槛。已有统计但样本不足时同样不命中本例分支。
...
每个 Score 使用 2—10 个有序等级，提交时检查此约束。解释器检查答案完整性、数值范围以及 confidence 的类型与范围，再执行组合。所有评分维度均为必需；缺失或非法回答使整次决策停止，不能当作零分。合法 confidence 包括 0，不设接受门槛，也不以它触发程序接管。
...
对抗环境可能需要随机化行动。V1.0 实现 `argmax` 和 `softmax_sample` 两种模式；不能仅接受配置而不执行。后者使用有限且大于零、位于运行时公开支持范围内的温度，按 `exp((utility - maxUtility) / temperature)` 归一化抽样，记录完整动作分布、实际动作概率及随机源标识。分数必须有限，归一化失败不能静默改成 `argmax`，应记录数值错误并停止。相同最高分在 `argmax` 下按领域固定优先级处理，softmax 中同分动作同概率。测试使用固定种子；实际运行使用独立、不可被对手预知的随机源。
...
3. **检查适用性**：核对规则、特征契约与资源限制；不满足则停止并报告错误。
4. **组织 Jev 问题**：投影状态，展开候选及维度，匹配条件并生成类型化问题。
5. **调用与校验**：在剩余时间内请求 Jev，校验答案 ID、类型、数值、置信度及模型信息。
6. **组合判断**：根据当前策略计算偏好并选择动作，记录匹配分支与随机化信息。
7. **提交前复核**：检查状态仍可执行，以幂等键提交；状态已变化则拒绝执行，交由调用者处理。
8. **记录与追踪反馈**：保存决策、回执及耗时，后续反馈异步追加并关联。

只有一个合法候选时也调用 Jev 并校验回答，成功后才能形成动作。所有动作统一经过执行模式、唯一所有者、新鲜度及持久意图检查。`shadow` 和仅返回决策的嵌入路径都不调用真实执行接口。没有合法候选时停止运行，不自行构造等待、弃牌或其他动作。

`DecisionRecord.decisionSource` 仅为 `strategy` 或 `stopped`。停止记录保存原策略摘要、`stopReason`、已有模型回答及用量，不含动作；SDK 抛出关联决策 ID 的错误，且不创建执行意图。停止记录不能提交执行入口。
...
超时、连接失败、答案缺失或非法、模型版本不符都会停止决策；不支持的策略格式明确拒绝。合法低置信度回答正常消费。模型失败使实例保持停止状态，不能用 start() 自动清除；宿主修复原因、核对外部执行并显式重建实例后再继续。
...
被测系统包括策略、实际模型、运行时、特征与知识构建及参考续打上下文。候选与对照均调用模型完成整条轨迹；任一模型失败中断本次实验，不允许用程序动作补齐或剔除失败样本后宣称改善。

### 5.4 公共运行与恢复语义

整个决策预算覆盖观察、模型请求和执行；状态过期时拒绝旧结果，不在失败路径偷偷重选动作。晚到或旧状态的模型结果可留作诊断，不能触发动作。实时、开发实验及最终实验的模型请求分别限制并发、预算和排队容量，测量实验并发对实时延迟的影响。
...
随机化选择器增加完整分布及动作概率比较断言，例如 `action_probability_decreases` 表达 `P_v2(bet | state) < P_v1(bet | state)`，不要求每次抽样都选择 `check`。检查非负有限概率、概率和在容差内为 1、同分同概率，以及选中动作和所记录概率的一致性。固定种子检查回放；跨种子抽样按预先固定的样本量与统计误差标准检查，不按单次动作判失败。温度边界、非有限分数和异常归一化另测明确停止原因。
...
| 实际修改 | 自动要求的检查 |
| --- | --- |
| 条件、权重或选择器；问题语义及输入不变 | 可复用绑定原问题摘要的固定回答，检查组合与动作变化；仍需完整对抗实验 |
| 问题、等级、维度语义、状态投影或参考续打假设 | 生成新的问题摘要；新语义夹具用于解释器测试，真实 Jev 调用验证评分质量及完整策略 |
| 增删维度 | 检查权重覆盖、问题预算、必需答案与停止行为，并按新问题重新取样 |
...
| 层次 | 检查内容 | 结论用途 |
| --- | --- | --- |
| 结构与语义 | 格式、特征及动作引用、条件类型、维度覆盖、数值、问题预算、停止路径 | 判断是否可执行 |
| 行为检查 | 固定局面与固定 Jev 回答下的分支、组合结果、预期动作变化和回归样例 | 判断策略差异是否按设计生效 |
| 对抗实验 | 新旧策略在相同条件下对多个对手的表现、尾部损失、时延及模型失败 | 判断是否有证据支持发布 |
...
建议的默认通过规则是：结构与行为检查通过；主要指标相对基线的配对差值置信区间下界超过预先设定的改善门槛；重要对手分组的退化不超过容许幅度；时延满足领域要求，且模型完成全部实验决策。置信水平、最小样本量和门槛都写入评价协议，不使用事后挑选的标准。
...
StrategyPackage + Jev 模型 + StrategyRuntime
  + 特征构建与知识更新实现 + 参考续打策略及其评分上下文
```

`ReleaseBinding` 保存这些依赖的不可变摘要（包括 `continuationPolicyDigest`），验证报告引用同一依赖清单。只修改策略的对比实验必须保持其他依赖一致；依赖也发生变化时，结果属于组合变更，不能全部归因于策略包。任何影响行为的产物变更都使原验证结论不再直接适用，包括修改模型失败停止逻辑或特征构建实现。

### 7.4 对外评价契约与报告

`EvaluationAdapter`、协议模板及指标接口属于公共领域扩展能力。领域声明指标名称、方向、单位、统计单位、窗口、未结算处理、分组和通过门槛；核心负责执行冻结协议，不内置“平均筹码收益上升”作为通用准入条件。

报告同时提供结构化结果与可读解释，标明真实/模拟模型调用、指标与不确定性、退化分组、费用、超时、停止原因、行为依赖及激活资格。保留评估明细与可公开结论分开导出，通用日志、动作解释和历史查询不能绕过研究侧的数据隔离。缺少独立评价能力时可以保存候选，不能把影子决策当作已实现收益或自动授予发布资格。
...
保留最近兼容且通过验证的稳定策略版本。上线后出现结构性故障或预设窗口内持续退化时，按切换规则回退。回退前核对规则、特征契约和 Jev 模型兼容性，不能无条件恢复任意历史版本。
...
**旧策略。** v1 的组合规则是 `gain - 0.2 × exposure`。夹具设定以下归一化评分，且回答完整合法；新旧版本的问题定义、状态和参考续打上下文均相同：
...
| 阶段 | 交付 | 完成标准 |
| --- | --- | --- |
| M0：验证 Jev 接入 | 直接 Choice、多维 Score 两条模型路径的真实对照 | 比较行为质量、收益、调用费用及延迟，确定适用提问方式 |
...
**M0 双模型路径对照。** 在相同初始局面、对手分布、知识状态和资源限制下比较：

| 路径 | 需要验证的问题 |
| --- | --- |
| Jev Choice 直接选候选 | 最简单的模型决策路径能达到怎样的质量与成本 |
| Jev 多维 Score＋策略组合 | 评分分解是否改善行为，增加了多少延迟、费用及失败风险 |

两条模型路径使用相同领域规则上下文、实际 Jev 模型版本和停止契约，各自固定问题定义。报告总体表现、完成状态及失败原因，不预设 Score 优于 Choice。Choice 对照只用于实验，若希望将其纳入可演化策略语言，需要另行定义结果消费语义。Kuhn 的结论首先证明闭环机制和可重复性；接入现有牌类 bot 后是否变强，属于新的适配与实验阶段。
...
6. Jev 超时、缺失答案、状态过期及执行回执不明时，均执行约定的停止或核对路径，不生成程序替代动作。
...
13. 后到的反馈修订不改变旧快照；失败记录能追踪到原策略和停止原因，行为依赖变更使原验证绑定失效。
...
| R10 性能边界 | 在声明领域、规模、并发、网络与预算下报告端到端 p50/p95/p99、超时、停止、费用、不确定性及实验并发影响 |
...
| 视角 | 核心信息 |
| --- | --- |
| 当前运行 | 生效策略、作用域、当前观察、候选动作、选择结果、耗时与停止原因 |
...
统一关联观察、决策、执行、反馈、研究、提交、验证和发布。用户应能查明为什么未进入模型调用、为什么停止、为什么研究未更新、为什么已验证但未激活，以及重启后为何等待核对。指标覆盖排队、端到端时延、超时、停止原因、研究/实验预算、待核对动作、存储健康和发布状态，不要求先有 Web 后台。
