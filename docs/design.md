...
第一版采用受约束的声明式策略语言，由确定性解释器执行。JSON 是规范存储格式，YAML 可作为编辑格式并转换为 JSON。策略不包含任意 JavaScript、Python、网络请求或文件操作。当前策略 Schema 为 `2.0`，评价协议为 `3.0`，运行时行为版本为 `duelloop-runtime-4`；旧字段与旧行为绑定明确拒绝，历史证据保留。
