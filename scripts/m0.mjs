...
export async function episode(path,seed,opponentId,options,model,budget) {
 ...
 let reward=0,fallbacks=0,timeouts=0,modelCalls=0;
 const usage=usageAccumulator();const reasons={};let failureCode=null;
 try {
 while(settled.size<options.hands) {
  ...
  let action;
  if(path==='domain_baseline')action=await domain.fallback(observation,'m0_control');
  else {
   ...
   try {
    ...
    if(path==='jev_choice') {
     ...
     if(answer.confidence<strategy.decision.minRequiredConfidence)throw Object.assign(Error('Low confidence'),{code:'MODEL_INVALID'});
    ...
    if(error.code==='VERSION_INCOMPATIBLE')throw error;
    const reason=error.code??'MODEL_INVALID';reasons[reason]=(reasons[reason]??0)+1;
    fallbacks++;if(reason==='MODEL_TIMEOUT')timeouts++;
    // A local rejection (for example low confidence) does not erase known response usage.
    if(!responseUsageRecorded)addUsage(usage,error.context?.usage);
    action=await domain.fallback(observation,reason);
   }
  }
  if(!action)throw Error('Domain has no legal fallback action');
 ...
 } catch(error){failureCode=error.code??'EXPERIMENT_ERROR';}
 const elapsedMs=performance.now()-started;
 return {status:failureCode?'incomplete':'completed',failureCode,seed,opponentId,path,hands:settled.size,rewardPerHand:settled.size?reward/settled.size:null,decisions:times.length,modelCalls,fallbacks,timeouts,fallbackReasons:reasons,elapsedMs,decisionsPerSecond:times.length/(elapsedMs/1000),latencyMs:quantiles(times),rawLatencyMs:times,usage};
}
export async function runM0(options) {
 ...
 outer:for(const seed of options.seeds)for(const opponent of options.opponents)for(const path of ['domain_baseline','jev_choice','jev_score']) {
 ...
 const paths=Object.fromEntries(['domain_baseline','jev_choice','jev_score'].map(path=>{
  ...
  return [path,{completedBlocks:values.filter(v=>v.status==='completed').length,hands:values.reduce((s,b)=>s+b.hands,0),meanRewardPerHand:values.reduce((s,b)=>s+b.hands,0)?values.reduce((s,b)=>s+(b.rewardPerHand??0)*b.hands,0)/values.reduce((s,b)=>s+b.hands,0):null,decisions,modelCalls:values.reduce((s,b)=>s+b.modelCalls,0),fallbackRate:decisions?values.reduce((s,b)=>s+b.fallbacks,0)/decisions:null,timeoutRate:decisions?values.reduce((s,b)=>s+b.timeouts,0)/decisions:null,decisionsPerSecond:duration?decisions/(duration/1000):null,latencyMs:quantiles(values.flatMap(b=>b.rawLatencyMs)),usage}];
 }));
 return {schemaVersion:'1.0',experiment:'M0-three-path-control',status,failureCode,modelCallsAttempted:budget.used,modelKind:model.kind,model:model.id,startedAt:started,node:process.version,platform:process.platform,conditions:{seeds:options.seeds,opponents:options.opponents,handsPerBlock:options.hands,knowledgeStateMode:'frozen',initialKnowledge:{},timeoutMs:options.timeoutMs,maxModelCalls:options.maxCalls,strategyDigest:digest(createKuhnStrategy()),environment:'Kuhn Poker simulation; every path receives an independent initial environment/knowledge/opponent instance'},claim:options.fixture?'Offline fixture mechanism/timing only; no real Jev latency or playing-strength claim':'Descriptive measurements on declared seeds/opponents; no independent holdout or production transfer claim',paths,blocks};
}
