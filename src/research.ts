...
const scoreAnswerSchema: Record<string, Json> = {type:'object',additionalProperties:false,required:['score','confidence','probabilities'],properties:{
  ...
  confidence:{type:'number',minimum:0,maximum:1,description:'Must meet the tested strategy minRequiredConfidence.'},
...
function submissionSchemas(run: ResearchRun, baseline: StrategyPackage): {strategy: Record<string,Json>; fixture: Record<string,Json>; submission: Record<string,Json>} {
  const strategy: Record<string,Json>={type:'object',additionalProperties:false,required:['schemaVersion','strategyId','version','parentVersion','scope','stateProjection','questions','decision','exitConditions','fallback','provenance'],properties:{
    schemaVersion:{const:'1.0'},strategyId:{const:baseline.strategyId},version:{...textSchema,description:`A new nonempty STRING, different from baseline version ${JSON.stringify(baseline.version)}. Versions are not JSON numbers.`},parentVersion:{const:baseline.version},scope:{const:jsonValue(baseline.scope)},
    ...
    decision:{type:'object',description:'Complete decision object in baseline format. Weights must cover all dimension IDs.'},exitConditions:{type:'array',items:{type:'object'}},fallback:{type:'object',additionalProperties:false,required:['mode'],properties:{mode:{const:'domain_baseline'}}},
...
export function validateSubmission(input: unknown, options: { run: ResearchRun; baseline: StrategyPackage; domain: DomainDefinition; evidenceRefs?: Set<string> }): { submission: CandidateSubmission; checkPlan: CandidateCheckPlan } {
  ...
  invariant(checkPlan.changedSections.some(x=>['questions','stateProjection','decision','exitConditions'].includes(x)), 'VALIDATION_REJECTED', 'Candidate changes metadata only');
...
export class ResearchOrchestrator {
  ...
  async run(id: string): Promise<ResearchResult> {
    ...
    try {
      ...
      const publicGoal = {runId:id,scopeId:initial.scopeId,baseReleaseDigest:initial.baseReleaseDigest,researchSnapshotId:initial.researchSnapshotId,evaluationProtocolDigest:initial.evaluationProtocolDigest,metric:protocol.metric,minimumImprovement:protocol.minimumImprovement,maxGroupRegression:protocol.maxGroupRegression,maxFallbackRate:protocol.maxFallbackRate,maxP95LatencyMs:protocol.maxP95LatencyMs,budget:this.budget};
...
function queryFrozenExperience(snapshot: Json, snapshotId: string, input: unknown): Json {
  ...
  const items=page.map(item=>{
    const keys=item.kind==='decision'?['decisionId','decisionSource','action','fallbackReason','observation','utilities','probabilities']:['feedbackId','revision','trajectoryId','eventTime','receivedAt','settled','metrics'];
