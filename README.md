...
0.2.3 增加受限的显式 bootstrap 模型维护，参见[运维说明](docs/operations.md#显式-bootstrap-模型维护)。0.2.2 为宿主事件驱动集成增加逐次取消、独立模型截止时间、提前轨迹绑定、稳定回执去重及首次结算研究触发。行为依赖升级为 runtime 5，SQLite 增量迁移到 Schema 3；详细边界与升级要求见 [实时契约](docs/live-decision-contracts.md)。
